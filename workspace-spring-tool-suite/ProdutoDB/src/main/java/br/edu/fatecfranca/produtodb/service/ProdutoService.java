...
public class ProdutoService {
	// vamos criar um objeto que vai conter todos os método de CRUD da classe Produto
	// a criação deste objeto não vai exigir o new, o que caracteriz o conceito de injeção de dependencia
	@Autowired
	ProdutoRepository injecao;
	
	// consulta no bando de daods é sempre o verbo GET
	@GetMapping
	public List<ProdutoDto> getProdutos(){
		List<Produto> produtos = injecao.findAll();
		return converteListaProdutosToListaDto(produtos);	
	}
	
	//consulta no banco de daods por um produto especifico
	@GetMapping("/{id}")
	...
	public ProdutoDto updateProduto(@RequestBody ProdutoDto produtoDto) {
		// perceba que o save está sendo utilizado na inserção e atualização. 
		// Isso porque, caso o produto não tenha id, é pra salvar, caso o produto
		// tenha id, vai atualizar.
		Produto produto = converteDtoToProduto(produtoDto);
	...
	public List<ProdutoDto> converteListaProdutosToListaDto(List<Produto> produtos){
